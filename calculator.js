    ...
    class Graph {
        constructor() {
            ...
            this.handleScroll = e => {
                ...
                if (wheelDeltaY > 0) {
                    this.orientation.zoom = +(this.orientation.zoom *
                        (1 + this.zoomPercentage)).toFixed(4);
                    this.orientation.x = Math.round(this.orientation.x - dX * zoomPanFactor);
                    this.orientation.y = Math.round(this.orientation.y - dY * zoomPanFactor);
                }
                else {
                    this.orientation.zoom = +(this.orientation.zoom /
                        (1 + this.zoomPercentage)).toFixed(4);
        ...
        plotGridSegment(direction) {
            const { gridSpacing, orientation: { x, y, zoom }, canvas: { width, height } } = this;
            let pt = direction === Directions.up || direction === Directions.down ? y : x;
            let label = 0;
            let index = 0;
        ...
        calculateBreakpoint() {
            const { maxGridCells, canvas: { width, height }, orientation: { zoom } } = this;
